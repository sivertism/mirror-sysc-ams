...
//begin implementation specific
	...
	virtual void bind(sc_core::sc_signal_inout_if<T>&);
...
private:
	...
	//begin implementation specific
	 ...
	 void bind( sc_core::sc_port_b<sc_core::sc_signal_inout_if<T> >& parent_ );
...
inline void sca_out<T>::start_of_simulation()
{
	...
	this->register_sca_schedule(this->get_timeoffset_internal(), value_handle);
...
{
	unsigned long delay_old=this->get_delay_internal();

	this->sca_tdf::sca_implementation::sca_port_attributes::set_delay(nsamples);

	std::vector<T> tmp(nsamples,T());
	if(pmod && pmod->is_change_attributes_executing())
	{
		for(unsigned long i=0;i<delay_old;i++)
		{
			if(i>=nsamples) break;
			...
			long csample=
					...
					* this->get_rate_internal() - i;

			tmp[(csample+nsamples)%nsamples]=delay_buffer[(csample+delay_old)%delay_old];
		}
	}
...
inline void sca_out<T>::initialize(const T& value, unsigned long sample_id)
{
	...
	sc_core::sc_time sched_time = sample_id * ptimestep + this->get_timeoffset_internal();

	if(pmod->is_reinitialize_executing())
	{
		unsigned long csample=
				pmod->get_call_counter() %
				pmod->get_calls_per_period()
				* this->get_rate_internal() + sample_id;
		delay_buffer[csample%delay]=value;
	}
	else
	{
		delay_buffer[sample_id%delay]=value;
	}

	value_handle.write_tmp(value);
	this->write_sc_value_on_time(sched_time, sched_time + ptimestep, value_handle);
}
...
inline const T& sca_out<T>::read_delayed_value(unsigned long sample_id) const
{
	...
	return delay_buffer[csample%delay];
}
...
inline void sca_out<T>::write(const T& value, unsigned long sample_id)
{
	...
	sca_core::sca_time sched_time = ctime + delay * ptimestep;

	if(delay>0)
	{
		unsigned long csample=
			pmod->get_call_counter() % pmod->get_calls_per_period()
			* this->get_rate_internal() + sample_id;
		delay_buffer[csample%delay]=value;
	}
