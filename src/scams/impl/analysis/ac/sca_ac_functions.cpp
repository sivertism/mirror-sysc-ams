...
/*****************************************************************************
  ...
  Created on: 03.01.2010
   ...
   SVN Id            :  $Id: sca_ac_functions.cpp 1907 2016-02-13 19:18:55Z karsten $
...
sca_util::sca_complex& sca_ac(const sc_core::sc_port_base& port)
{
    ...
    std::ostringstream str;
    str << "AC-simulation - access to a port which is not connected to "
    << "a signal registrated as ac-arc: " << port.name() << std::endl;
    SC_REPORT_ERROR("SystemC-AMS",str.str().c_str());

    return dummy_complex; //never happens - against compiler warning
}
...
sca_util::sca_complex& sca_ac_noise(const sc_core::sc_port_base& port, std::string src_name)
{
    ...
    std::ostringstream str;
    str << "AC-Noise-simulation - access to a port which is not connected to "
    << "a signal registrated as ac-arc: " << port.name() << std::endl;
    SC_REPORT_ERROR("SystemC-AMS",str.str().c_str());
