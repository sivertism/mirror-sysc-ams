...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_tdf_ct_vector_ss_proxy.cpp 1920 2016-02-25
 12:43:37Z karsten $
...
sca_ct_vector_ss_proxy::sca_ct_vector_ss_proxy(::sc_core::sc_object* object) {
  ...
  if (!parent_module) {
    ...
    if (object != NULL) str << object->name() << ":";
    str << "\n\tA "
        << "sca_tdf::sca_ltf_ss"
...
inline bool sca_ct_vector_ss_proxy::matrix_changed(
  ...
  if (matrix.get_access_flag() || (&matrix_old != matrix_old_ref)) {
    ...
    if ((matrix.n_cols() != matrix_old.n_cols()) ||
        (matrix.n_rows() != matrix_old.n_rows())) {
      return true;
    }
    ...
    for (unsigned int i = 0; i < matrix.n_cols(); i++)
      for (unsigned int j = 0; j < matrix.n_rows(); j++) {
        if (matrix(j, i) != matrix_old(j, i)) return true;
      }
  }
...
inline bool sca_ct_vector_ss_proxy::coeff_changed(
  ...
  if (matrix_changed(a, a_old, a_old_ref) ||
      matrix_changed(b, b_old, b_old_ref) ||
      matrix_changed(c, c_old, c_old_ref) ||
...
inline void sca_ct_vector_ss_proxy::setup_equation_system() {
  ...
  new_memsize =
      2 * number_of_equations * number_of_equations + number_of_equations;

  if (new_memsize != memsize) {
    if (q_ss != NULL) delete[] q_ss;
...
inline void sca_ct_vector_ss_proxy::initialize_equation_system(int init_mode,
  ...
  if (number_of_equations > 0) {
    int err = ana_reinit(a_ss, b_ss, number_of_equations, h, &sdata, init_mode);
    if (err) {
      std::ostringstream str;
      str << ltf_object->name()
...
inline void sca_ct_vector_ss_proxy::initialize() {
  ...
  if (ay != by) {
    std::ostringstream str;
    str << ltf_object->name()
        << ":\n\tDimension mismatch with matrices A and B." << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  if ((ax != cx) & (cy > 0)) {
    std::ostringstream str;
    str << ltf_object->name()
        << ":\n\tDimension mismatch with matrices A and C." << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }

  if (ax != ay) {
    std::ostringstream str;
    str << ltf_object->name() << ":\n\tMatrix A must be square." << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  ...
  if ((cy != dy) & (cx > 0)) {
    std::ostringstream str;
    str << ltf_object->name()
        << ":\n\tDimension mismatch with matrices C and D." << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  ...
  if ((bx != dx) & (by > 0)) {
    std::ostringstream str;
    str << ltf_object->name()
        << ":\n\tDimension mismatch with matrices B and D." << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  ...
  if (state_size != s_size) {
    std::ostringstream str;
    str << ltf_object->name() << ":\n\tThe size of the state vector (" << s_size
        << ") does not match the number of equations (e.g. dimension of square "
           "matrix A: "
        << state_size << ")." << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
...
inline sca_util::sca_vector<double> sca_ct_vector_ss_proxy::calculate(
  ...
  unsigned long nout = (c_old.n_rows() == 0) ? d_old.n_rows() : c_old.n_rows();
...
inline sca_util::sca_vector<double>
sca_ct_vector_ss_proxy::get_in_value_by_index(unsigned long k) {
  switch (intype) {
    case IN_SCALAR:
      return in_scalar;
    case IN_VECTOR: {
    ...
    case IN_SC_PORT:
      return const_cast<
                 sca_tdf::sca_de::sca_in<sca_util::sca_vector<double> >*>(
                 in_sc_port)
          ->read(k);
    default:
...
inline void sca_ct_vector_ss_proxy::register_ss_common(
    ...
    const sca_util::sca_matrix<double>& b,
    const sca_util::sca_matrix<double>& c,
    const sca_util::sca_matrix<double>& d, const sca_core::sca_time& tstep) {
  // check, that the corresponding cast operator for the last call was executed
  if (pending_calculation) {
    std::ostringstream str;
    str << "The previous ltf_nd calculation was performed without assigning"
           " the result for: "
        << ltf_object->name();
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  ...
  // if there is a delay or we have more than one sample we create a delay
  // buffer
  if ((delay_buffer == NULL) &&
      ((ct_in_delay > sc_core::SC_ZERO_TIME) || (number_of_in_values > 1))) {
    delay_buffer = new sca_ct_delay_buffer<sca_util::sca_vector<double> >;
  ...
  if (ccnt != module_activations)  // first activation of current module call
  {
    ...
    {               // we do not update module_activations
      last_calculated_in_time =
          sc_core::SC_ZERO_TIME - in_time_step * number_of_in_values;
      ...
      if (module_is_dynamic) {
        ...
        if (module_time_step == sc_core::SC_ZERO_TIME) {
          // the module timestep becomes the timestep of the last module
          // execution
          ...
          if (s_intern.length() > 0)
            ana_restore_solver_check_point(sdata, s_intern.get_flat(),
                                           &s_intern_module_backup);
          ...
          if (iterations_enabled) {
            ...
            ana_copy_solver_check_point(&s_intern_tstep_backup,
                                        s_intern_tstep_module_backup);
            last_in_time_tstep_backup = last_in_time_module_tstep_backup;
            last_out_time_tstep_backup = last_out_time_module_tstep_backup;
            last_calculated_time_tstep_backup =
                last_calculated_time_module_tstep_backup;
        ...
        } else  // the module time progresses
        {
          if (iterations_enabled) {
            ...
            ana_copy_solver_check_point(&s_intern_tstep_module_backup,
                                        s_intern_tstep_backup);
          ...
          if (s_intern.length() > 0)
            ana_store_solver_check_point(sdata, s_intern.get_flat(),
                                         &s_intern_module_backup);
          last_in_time_module_backup = last_calculated_in_time;
          last_out_time_module_backup = last_out_time;
          last_calculated_time_module_backup = last_calculated_time;
      ...
      if (module_time_step <
          module_current_time) {  // calculate only if current time > 0
        remainig_time =
            module_current_time - module_time_step - last_calculated_in_time;
      }
      first_in_time_step = remainig_time + in_time_step;

      // used for new module activation detection (if not equal->new module
      // activation)
      module_activations = ccnt;

      // store/restore solver checkpoint due tstep=0
      // tstep=0 and no remaining time for the first ltf activation of a module
      // call
      if (iterations_enabled) {
        ...
        if (first_in_time_step == sc_core::SC_ZERO_TIME) {
          ...
          // we  are at the start of the current interval (the current time is
          // the end)
          if (module_time_step < module_current_time) {
            remainig_time = module_current_time - last_calculated_in_time -
                            module_time_step;
          }
  ...
  else  // following calculation in the same module activation
  {
    if (iterations_enabled) {
      ...
      if (time_interval == sc_core::SC_ZERO_TIME) {
        ...
        {
          // the time interval is the difference between the last and the
          // current
          time_interval =
              last_calculated_time - last_calculated_time_tstep_backup;
  ...
  if ((first_in_time_step == sc_core::SC_ZERO_TIME) ||
      (ccnt == 0)) {  // all activation in the first call considered as dc-steps
    dc_init = true;
  }
  ...
  if (current_time + time_interval > module_time_step) {
    ...
    str << "The requested timestep ( " << tstep
        ...
        << ltf_object->name() << " ltf time since activation: " << current_time
        << " module timestep: " << module_time_step;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  ...
  if ((delay_buffer == NULL) || (max_delay == sc_core::SC_ZERO_TIME)) {
    if (ct_in_delay > time_interval) {
      std::ostringstream str;
      str << "The parameter ct_delay (" << ct_in_delay
          << ") of: " << ltf_object->name()
  ...
  } else  // delay_buffer != NULL
  {
    if (ct_in_delay > max_delay) {
      ...
      str << "The parameter ct_delay (" << ct_in_delay
          << ") of: " << ltf_object->name()
          << " must be smaller or equal than the maximum delay: " << max_delay
...
void sca_ct_vector_ss_proxy::register_ss(const sca_util::sca_matrix<double>& a,
  ...
  if (delay_buffer != NULL) {
    // current_time is time since current module activation
    sca_core::sca_implementation::sca_signed_time ctime =
        last_available_in_time;
    for (unsigned i = 0; i < number_of_in_values; i++) {
      sca_core::sca_implementation::sca_signed_time stime =
          (number_of_in_values - 1 - i) * in_time_step;
...
inline void sca_ct_vector_ss_proxy::register_ss_port(
    ...
    const sca_util::sca_matrix<double>& b,
    const sca_util::sca_matrix<double>& c,
    const sca_util::sca_matrix<double>& d, sca_core::sca_time& ct_delay,
    sca_util::sca_vector<double>& state, unsigned long port_rate,
    sca_core::sca_time& first_port_timestep, sca_core::sca_time& port_timestep,
  ...
  if (delay_buffer != NULL) {
    ...
    for (unsigned long i = 0; i < port_rate; i++) {
      ...
      if (ctime >= current_time) {
        delay_buffer->store_value(module_current_time + ctime,
                                  get_in_value_by_index(i));
      }
      ctime += port_timestep;
    }
  }
  ...
  last_available_in_time =
      module_current_time + (port_rate - 1) * port_timestep;
  current_time += time_interval;
...
void sca_ct_vector_ss_proxy::register_ss(
  ...
  register_ss_port(a, b, c, d, ct_delay, state, port_rate, first_port_timestep,
                   port_timestep, port_value);
}
...
void sca_ct_vector_ss_proxy::register_ss(
  ...
  sca_core::sca_time port_timestep = first_port_timestep;
  sca_util::sca_vector<double> port_value =
      const_cast<sca_tdf::sca_de::sca_in<sca_util::sca_vector<double> >*>(&x)
          ->read();
...
void sca_ct_vector_ss_proxy::register_ss(const sca_util::sca_matrix<double>& a,
                                         ...
                                         const sca_util::sca_vector<double>& x,
                                         sca_core::sca_time tstep) {
  if (s_intern.length() != state_size) s_intern.resize(0);
...
const sca_util::sca_vector<double>&
sca_ct_vector_ss_proxy::convert_to_double_vector() {
  if (!pending_calculation) {
    std::ostringstream str;
  ...
  if (delay_buffer == NULL) {
    ...
    } else  // if delay - more complicated (effort)
    {
      ...
      {
        if (first_step)  // the first value must be the initial value (before
                         // invalue)
        {
          in_val[i] = tmp[i];
        } else {
          in_val[i] =
              (tmp[i] - in_last[i]) / (first_in_time_step.to_seconds()) *
                  (first_in_time_step.to_seconds() - ct_in_delay.to_seconds()) +
              in_last[i];
        }
      }
    ...
    if (number_of_in_values > in_index_offset + 1) {
      h = in_time_step
              .to_seconds();  // calculate following timesteps for in values

      // if required (h changed) re-initialize equation system
      initialize_equation_system(2, h);

      for (unsigned long i = in_index_offset + 1; i < number_of_in_values;
           i++) {
  ...
  } else  // delay_buffer!=NULL
  {
    ...
    while (true) {
      ...
      bool no_next_value = delay_buffer->get_next_value_after(
          sample_timestamp, value, last_calculated_time - ct_in_delay);
      sca_core::sca_implementation::sca_signed_time value_timestamp =
          sample_timestamp + ct_in_delay;

      // the last value in buffer is equal to the requested out timestamp
      if (value_timestamp == next_out_time) {  // calculate outvalue
      ...
      if (no_next_value) {
        ...
        str << "Internal error due a bug: " << last_calculated_time
            ...
            << " last out time: " << last_out_time << " time interval "
            << time_interval;
        SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
        return scalar_out;
      }
      ...
      if ((value_timestamp < next_out_time) &&
          (value_timestamp >=
           sc_core::SC_ZERO_TIME)) {  // integrate to invalue - ignore values
                                      // with negative time
        h = (value_timestamp - last_calculated_time).to_seconds();
        initialize_equation_system(2, h);
        calculate(value);
...
inline void sca_ct_vector_ss_proxy::calculate_timeinterval(
    ...
    long number_of_out_values,
    sca_core::sca_implementation::sca_signed_time& next_in_time,
    sca_core::sca_implementation::sca_signed_time& next_out_time,
    sca_core::sca_time& next_in_time_step, sca_core::sca_time& out_time_step) {
  if (!pending_calculation) {
    std::ostringstream str;
  ...
  if (delay_buffer == NULL) {
    ...
    while (current_out_index < number_of_out_values) {
      ...
      if (sc_dt::sc_abs(
              sc_dt::int64((next_in_time.value() + ct_in_delay.value() -
      ...
      // next time in value only -> ignore result
        ...
        sca_core::sca_time dt =
            next_in_time + ct_in_delay - last_calculated_time;
        h = (dt == sc_core::SC_ZERO_TIME) ? in_time_step.to_seconds()
                                          : dt.to_seconds();
        ...
        next_in_time += next_in_time_step;
        next_in_time_step = in_time_step;
      } else  // next time calculate out value -> interpolate in (in time after
              // next out time)
      {
        sca_core::sca_time dt =
            next_out_time + ct_in_delay - last_calculated_time;
        h = (dt == sc_core::SC_ZERO_TIME) ? in_time_step.to_seconds()
                                          : dt.to_seconds();
        ...
        unsigned long dimy =
            c_old.n_rows() > 0 ? c_old.n_rows() : d_old.n_rows();
        sca_util::sca_vector<double> inp(dimy);
        sca_util::sca_vector<double> tmp =
            get_in_value_by_index(current_in_index);

        for (unsigned long i = 0; i < dimy; i++) {
          inp[i] = (tmp[i] - in_last[i]) / (next_in_time_step.to_seconds()) *
                       (next_out_time - last_calculated_in_time).to_seconds() +
                   in_last[i];
        }
      ...
      std::cout << "ct: " << last_calculated_time << "  not: " << next_out_time
                << " nit: " << next_in_time << " coi: " << current_out_index
                << " cii: " << current_in_index
                << " last_in_time: " << last_calculated_in_time << " h: " << h
                << std::endl;
#endif
  ...
  } else  // delay_buffer!=NULL
  {
    ...
    sca_core::sca_implementation::sca_signed_time last_delay_time =
        last_calculated_time - ct_in_delay;
    ...
    while (outcnt < number_of_out_values) {
      ...
      if ((!delay_buffer->get_next_value_after(ntime, value,
                                               last_delay_time)) &&
      ...
      {
        if (ntime !=
            (next_out_time - ct_in_delay)) {  // if the next value is after the
                                              // next out time, we must
          // interpolate
          if (delay_buffer->get_value(next_out_time - ct_in_delay, value)) {
            ...
            std::ostringstream str;
            str << "Internal Error due a Bug ";
            str << "next_out_time: " << next_out_time
                << " ct_in_delay: " << ct_in_delay
                << " next_out_time-ct_in_delay: " << next_out_time - ct_in_delay
                << " in: " << ltf_object->name();
            SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
          }
...
inline void sca_ct_vector_ss_proxy::convert_to_port(
    sca_core::sca_time& first_port_timestep, sca_core::sca_time& port_timestep,
    sca_core::sca_time& port_time, unsigned long port_rate) {
  ...
  if ((int_next_out_time <= int_last_out_time) && (int_last_out_time != 0)) {
    current_out_index =
        (int_last_out_time - int_next_out_time + out_time_step.value() - 1) /
            out_time_step.value() +
        1;
...
#ifdef SCA_DEBUG_LTF_SS
    std::cout << this->ltf_object->name() << " oend; " << out_end
              << " iend: " << in_end << " out_time_step: " << out_time_step
              << " port_rate: " << port_rate
              << " next_in_time: " << next_in_time << "  number_of_in_values  "
              << number_of_in_values << " in_time_step: " << in_time_step
              << " last_in_time " << last_calculated_in_time
              << " first_in_time_step  " << first_in_time_step
              << " current_in_index: " << current_in_index
              << " next_out_time: " << next_out_time << " current_out_index "
              << current_out_index << std::endl;
#endif
    if (out_end > in_end) {
      ...
      std::ostringstream str;
      str << "The input time intervall ends before the requested output time "
  ...
  calculate_timeinterval(current_in_index, current_out_index, port_rate,
                         next_in_time, next_out_time, next_in_time_step,
                         out_time_step);
}
...
void sca_ct_vector_ss_proxy::convert_to_matrix(
  ...
  sca_core::sca_time first_out_time_step =
      time_interval - (number_of_out_samples - 1) * out_time_step;
  ...
  {
    next_out_time =
        sc_core::SC_ZERO_TIME - out_time_step * (number_of_out_samples - 1);
...
void sca_ct_vector_ss_proxy::enable_iterations() {
  if (sca_core::sca_implementation::sca_get_curr_simcontext()
          ->elaboration_finished()) {
    std::ostringstream str;
...
sca_util::sca_vector<double> sca_ct_vector_ss_proxy::estimate_next_value() {
  ...
  if (statep != NULL) {
    ...
    ana_restore_solver_check_point(sdata, statep->get_flat(),
                                   &estimate_cp_data);
  }
