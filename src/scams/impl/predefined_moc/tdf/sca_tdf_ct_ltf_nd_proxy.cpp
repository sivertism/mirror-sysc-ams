...
/*****************************************************************************
 ...
 SVN Version       :  $Revision: 1828 $
 SVN last checkin  :  $Date: 2015-05-18 16:59:47 +0200 (Mon, 18 May 2015) $
 SVN checkin by    :  $Author: karsten $
 SVN Id            :  $Id: sca_tdf_ct_ltf_nd_proxy.cpp 1828 2015-05-18 14:59:47Z
 karsten $
...
inline bool sca_ct_ltf_nd_proxy::coeff_changed(
...
inline void sca_ct_ltf_nd_proxy::setup_equation_system() {
  ...
  if (new_memsize != memsize) {
    if (num2_ltf != NULL) delete[] num2_ltf;
  ...
  // the fractional and non-fractional part will be put to the same equation
  // system
  A.resize(number_of_all_equations, number_of_all_equations);
  B.resize(number_of_all_equations, number_of_all_equations);

  memset((void*)(num2_ltf), 0, new_memsize * sizeof(double));  // set all to
                                                               // zero
  ...
  if (number_of_equations2 > 0) {
    ...
    for (long i = long(num_size) - 1; i >= long(number_of_equations); --i) {
      ...
      for (long j = start; j < i; ++j) num2_ltf[j] -= c * den_ltf[j - start];
    }

    B(number_of_equations + state_size - den_size, number_of_equations) = 1.0;
    for (unsigned long i = number_of_equations + 1; i < number_of_all_equations;
         ++i) {
      A(i, i) = 1.0;
      B(i - 1, i) = -1.0;
    }
...
inline void sca_ct_ltf_nd_proxy::initialize_equation_system(int init_mode,
  ...
  if (number_of_all_equations > 0) {
    int err = ana_reinit_sparse(A.get_sparse_matrix(), B.get_sparse_matrix(), h,
                                &sdata, init_mode);
...
inline void sca_ct_ltf_nd_proxy::initialize() {
  ...
  if (nn == 0) {
    std::ostringstream str;
    str << ltf_object->name() << ":\n\tAn empty numerator does not make sense."
        << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
  ...
  if (state_size != s_size) {
    ...
    str << ltf_object->name() << ":\n\tThe size of the state vector (" << s_size
        << ") does not match the number of equations (" << state_size << ")."
        << std::endl;
    SC_REPORT_ERROR("SystemC-AMS", str.str().c_str());
  }
...
inline double sca_ct_ltf_nd_proxy::calculate(double invalue) {
  ...
  if (number_of_equations2 > 0) {
    for (unsigned long i = number_of_equations,
                       j = state_size - number_of_equations - 1;
         i < state_size; ++i, --j) {
...
inline double sca_ct_ltf_nd_proxy::get_in_value_by_index(unsigned long k) {
  switch (intype) {
    ...
    case IN_SC_PORT:
      return const_cast<sca_tdf::sca_de::sca_in<double>*>(in_sc_port)->read(k);
    default:
...
inline void sca_ct_ltf_nd_proxy::register_nd_common(
    const sca_util::sca_vector<double>& num,
    const sca_util::sca_vector<double>& den, const sca_core::sca_time& tstep) {
...
void sca_ct_ltf_nd_proxy::register_nd(const sca_util::sca_vector<double>& num,
                                      ...
                                      double k, sca_core::sca_time tstep) {
...
inline void sca_ct_ltf_nd_proxy::register_nd_port(
    ...
    sca_util::sca_vector<double>& state, double k, unsigned long port_rate,
...
void sca_ct_ltf_nd_proxy::register_nd(const sca_util::sca_vector<double>& num,
  ...
  register_nd_port(num, den, ct_delay, state, k, port_rate, first_port_timestep,
...
void sca_ct_ltf_nd_proxy::register_nd(const sca_util::sca_vector<double>& num,
                                      const sca_util::sca_vector<double>& den,
                                      sca_core::sca_time ct_delay, double input,
                                      double k, sca_core::sca_time tstep) {
  ...
  if (delay_buffer == NULL) {
    if (ct_in_delay == sc_core::SC_ZERO_TIME) {
      ...
      if (first_step)  // if value before time zero
      {
        ...
        in_val =
            (tmp - in_last) / (first_in_time_step.to_seconds()) *
  ...
  if (delay_buffer == NULL) {
    ...
    while (current_out_index < number_of_out_values) {
              ...
              // next out time)
      {
        ...
        inp = (tmp - in_last) / (next_in_time_step.to_seconds()) *
                  (next_out_time - last_calculated_in_time + ct_in_delay)
                      .to_seconds() +
              in_last;
...
inline void sca_ct_ltf_nd_proxy::convert_to_port(
  ...
  if (calculate_to_end) {
    ...
    std::cout << this->ltf_object->name() << " oend; " << out_end
              ...
              << " current_in_index: " << current_in_index << std::endl;
#endif
    if (out_end > in_end) {
...
void sca_ct_ltf_nd_proxy::convert_to_sca_port(
    sca_tdf::sca_out_base<double>& port) {
...
void sca_ct_ltf_nd_proxy::convert_to_vector(sca_util::sca_vector<double>& vec,
  ...
  if (nsamples == 0) {
    if (vec.length() == 0) {  // if the vector size is zero number of out
                              // samples equals number of
      // in samples
      vec.resize(number_of_in_values);
    } else {  // else it's the vector length
...
double sca_ct_ltf_nd_proxy::estimate_next_value() {
  ...
  if (statep != NULL) {
    double estimated_in;
    estimated_in = last_calculated_in +
                   last_delta_in * time_interval.to_seconds() / last_h;
