...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_tdf_port_attributes.cpp 1920 2016-02-25 12:43:37Z karsten $
...
sca_core::sca_time sca_port_attributes::get_timestep_internal(unsigned long sample_id) const
{
	...
	if(pmod->get_sync_domain()->get_cluster_synchronization_data()->attributes_changed)
	{
		...
		//the first sample timestep after a change is may be different
...
{
	return (get_delay_internal() != get_last_delay_internal());
}
...
void sca_port_attributes::set_rate(unsigned long rate_)
{
	...
	{
		std::ostringstream str;
		str << "Attribute rate is set to a very large value, may be a negative"
