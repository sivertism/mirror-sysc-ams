...
#else
	add_pwl_b_stamp_to_B(y1, x, ctrl, sca_util::sca_create_pair_vector(threshold.get() - 1.0, 0.0,  threshold.get(), 1.0 ));
	add_pwl_b_stamp_to_B(y2, x, ctrl, sca_util::sca_create_pair_vector(threshold.get() - 1.0, 1.0,  threshold.get(), 0.0 ));
#endif
}
