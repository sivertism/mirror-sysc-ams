...
namespace sca_eln
{
sca_vcc::sca_vcc(sc_core::sc_module_name,
...
const char* sca_vcc::kind() const
{
	return "sca_eln::sca_vcc";
}


void sca_vcc::trace( sc_core::sc_trace_file* tf ) const
{
	std::ostringstream str;
	str << "sc_trace of sca_vcc module not supported for module: " << this->name();
	SC_REPORT_WARNING("SystemC-AMS",str.str().c_str());
}


void sca_vcc::matrix_stamps()
{
	    ///////////////////////////////////////////////////////////////
		...
		if(!sca_ac_analysis::sca_ac_is_running() && dc_init)
		{
			...
			add_method(POST_SOLVE, SCA_VMPTR(sca_vcc::post_solve));
...
void sca_vcc::post_solve() //set capacitor stamps after first step
{
	//reset dc stamps
    ...
    remove_method(POST_SOLVE, SCA_VMPTR(sca_vcc::post_solve));
...
bool sca_vcc::trace_init(sca_util::sca_implementation::sca_trace_object_data& data)
{
...
void sca_vcc::trace(long id,sca_util::sca_implementation::sca_trace_buffer& buffer)
{
    sca_core::sca_time ctime = sca_eln::sca_module::get_time();

    buffer.store_time_stamp(id,ctime,x(nadd1));
}

sca_util::sca_complex sca_vcc::calculate_ac_result(sca_util::sca_complex* res_vec)
{
...
*/
void sca_vcc::set_unit(const std::string& unit_)
{
	unit=unit_;
}

const std::string& sca_vcc::get_unit() const
{
	return unit;
}

void sca_vcc::set_unit_prefix(const std::string& prefix_)
{
	unit_prefix=prefix_;
}

const std::string& sca_vcc::get_unit_prefix() const
{
	return unit_prefix;
}

void sca_vcc::set_domain(const std::string& domain_)
{
	domain=domain_;
}

const std::string& sca_vcc::get_domain() const
{
	return domain;
}
