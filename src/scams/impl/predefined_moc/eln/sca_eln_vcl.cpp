...
namespace sca_eln
{
sca_vcl::sca_vcl(sc_core::sc_module_name,
...
const char* sca_vcl::kind() const
{
	return "sca_eln::sca_vcl";
}


void sca_vcl::trace( sc_core::sc_trace_file* tf ) const
{
	std::ostringstream str;
	str << "sc_trace of sca_vcl module not supported for module: " << this->name();
	SC_REPORT_WARNING("SystemC-AMS",str.str().c_str());
}


void sca_vcl::matrix_stamps()
{
	...
	if(!sca_ac_analysis::sca_ac_is_running() && dc_init)
	{
		...
		add_method(POST_SOLVE, SCA_VMPTR(sca_vcl::post_solve));
...
void sca_vcl::post_solve() //set capacitor stamps after first step
{
	//reset dc stamps
	...
	remove_method(POST_SOLVE, SCA_VMPTR(sca_vcl::post_solve));
...
bool sca_vcl::trace_init(sca_util::sca_implementation::sca_trace_object_data& data)
{
...
void sca_vcl::trace(long id,sca_util::sca_implementation::sca_trace_buffer& buffer)
{
...
sca_util::sca_complex sca_vcl::calculate_ac_result(sca_util::sca_complex* res_vec)
{
...
*/
void sca_vcl::set_unit(const std::string& unit_)
{
	unit=unit_;
}

const std::string& sca_vcl::get_unit() const
{
	return unit;
}

void sca_vcl::set_unit_prefix(const std::string& prefix_)
{
	unit_prefix=prefix_;
}

const std::string& sca_vcl::get_unit_prefix() const
{
	return unit_prefix;
}

void sca_vcl::set_domain(const std::string& domain_)
{
	domain=domain_;
}

const std::string& sca_vcl::get_domain() const
{
	return domain;
}
