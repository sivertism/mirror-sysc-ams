...
/*****************************************************************************
 ...
 SVN Id            :  $Id: sca_eln_sc_rswitch.cpp 1936 2016-03-02 16:59:19Z karsten $
...
void sca_rswitch::trace( sc_core::sc_trace_file* tf ) const
{
	...
	str << "sc_trace of sca_ln::sca_de::sca_rswitch module not supported for module: " << this->name();
...
void sca_rswitch::update_rval()
{
	...
	sca_core::sca_time cur_time=this->get_time();
	if (cur_time == sc_core::SC_ZERO_TIME)
	{
		read_rval();
	}
	else
	{   //do not update for zero timestep
		if(last_time==cur_time) return;
	}
	last_time=cur_time;
