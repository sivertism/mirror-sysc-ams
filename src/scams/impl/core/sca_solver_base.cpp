...
void sca_solver_base::push_back_sync_port(sca_port_base* port, sca_port_direction_enum dir,
		sca_interface* ch, unsigned long& rate, unsigned long& delay,
		...
		::sc_core::sc_time next_time, sca_sync_value_handle_base& val_handle)
{
	...
	//limit sc time progress to next_time - switch to sca-domain
	if (((csync_data->cluster_resume_event_time) > next_time)
			|| ((csync_data->cluster_resume_event_time) <= sc_core::sc_time_stamp()))
	//instead sc_time_stamp() ctime should be "more correct" - however with "slight"
	//non causalities may a non obsolete limitation will be deleted - due this difficulties
	//we not further synchronize to the cluster_resume_event in get_sc_value_by_time()
	{
		csync_data->cluster_resume_event.cancel();//remove due earliest notification wins (LRM)
		csync_data->cluster_resume_event.notify(next_time - sc_core::sc_time_stamp());
		csync_data->cluster_resume_event_time = next_time;
	}
		...
		sca_sync_value_handle_base& val_handle)
{
	...
	if (ctime > ::sc_core::sc_time_stamp())
	{
		...
		//this should be sure - however causes may be some additional context switches
...
//     process is suspended, after resume the value is returned via the handle
//
void sca_solver_base::get_sc_value_by_order(
		sca_sync_value_handle_base& val_handle)
{
	sca_sync_trace_handle& handle(sync_trace_handles[val_handle.get_id()]); //get handle
	...
	if (handle.count < 1)
	{
		...
		{
			std::ostringstream str;
			str << "Internal error sca-de synchronization failed in: "
					<< __FILE__ << " line: " << __LINE__ << std::endl;
			SC_REPORT_ERROR("SystemC-AMS",str.str().c_str());
		}
	}

	val_handle.set_index(handle.index_start);
...
void sca_solver_base::register_sca_schedule(::sc_core::sc_time next_time,
		sca_sync_value_handle_base& handle)
{
	//initialize time on which the sdf cluster must be resumed due it
	//may be produces a sc-event
	if ((next_time != NOT_VALID_SCA_TIME()) && (next_time
			< csync_data->cluster_resume_event_time))
	{
		csync_data->cluster_resume_event_time = next_time;
		csync_data->cluster_resume_event.notify(next_time - sc_core::sc_time_stamp());
	}
...
sca_core::sca_time sca_solver_base::get_current_period()
{
	if ((csync_data == NULL) || (calls_per_period == NULL))
	 ...
	 //first cluster call has may be different timestep - if request_next_activation was called
