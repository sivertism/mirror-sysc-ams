...
void sca_trace_buffer::store_abstract_time_stamp(long id,
		const sca_core::sca_time& ctime, sca_trace_value_handler_base& value)
{
	...
	if (init_values[id] == NULL)
	{
		...
		init_values[id]->next_time=ctime;
	...
	value_buffer* ele = &(trace_buffer[ctime]);
	ele->last_values = &last_values;
	value.this_time = ctime;
	...
	if ((*ele)[id] != NULL)
	{
		(*ele)[id]->copy_value(value); //overwrite value
		delete &value;
		return;
	}
	else
	...
	if (last_stored_values[id] != NULL)
	{
		...
		last_stored_values[id]->next_time = ctime;
	}
...
bool sca_trace_buffer::read_next_stamp(sca_core::sca_time& ctime,
		value_buffer*& values)
{
	...
	for (unsigned long i = 0; i < number_of_traces; ++i)
	{
		...
		if (last_values[i] != NULL)
		{
			...
			if(last_values[i]->next_value==NULL) last_stored_values[i]=NULL;
...
bool sca_trace_buffer::read_sample_interpolate(const sca_core::sca_time& ntime,
		value_buffer*& values)
{
	values = NULL;
		...
		value_buffer*& values, bool hold_sample)
{
	...
	//if no timestamp than there are no traces added
	if (buf_it == trace_buffer.end())
	{
		values=NULL;
		return true;
