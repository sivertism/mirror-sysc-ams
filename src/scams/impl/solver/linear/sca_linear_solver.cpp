...
//   x3 -> segment 2
//          :
void sca_linear_solver::init_pwl_data()
{
	...
	for(unsigned long i=0; i<num_of_pwl_coeffs;++i)
	{
		...
		unsigned long idx=0;
		for(idx=0;idx<n_segments-1;++idx)
		{
			//first and last point must not be checked, due before is segemnt 0
			//and after remains the last segment
			if(data.x_values[idx+1]>=0.0) break;
		}
...
void sca_linear_solver::solve_eq_system()
{
	...
	{
		if((q_last==NULL) || (q_next==NULL))
		{
			double* q_cur=q->get_calc_flat();
			if(q_last==NULL)
			{
				q_last=new double[q->length()];
				for(unsigned long i=0; i<q->length();i++) q_last[i]=q_cur[i];
			}
	...
	else
	{
		...
		{
			//store last q
			for(unsigned long i=0; i<q->length();i++) q_last[i]=q_next[i];
		}
	}
