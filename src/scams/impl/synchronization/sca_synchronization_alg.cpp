...
void sca_synchronization_alg::cluster()
{
	...
	for_each(sync_objs.begin(),sync_objs.end(),
	              std::mem_fun(&sca_synchronization_obj_if::reset));
	...
	while (sync_obj != sync_objs.end())
	{
		...
		sync_obj = find_if(sync_obj, sync_objs.end(), std::mem_fun(
		...
		sca_synchronization_alg::sca_cluster_objT& cluster)
{
	...
	std::sort(schedule_vector.begin(),schedule_vector.end(),
		   std::mem_fun(&schedule_element::less_time));
	...
	std::copy(schedule_vector.begin(), schedule_vector.end(), tmp_list.begin());
	...
	while (true)
	{
		...
		lslit=std::find_if(tmp_list.begin(),tmp_list.end(),
		                  std::mem_fun(&schedule_element::schedule_if_possible));
	...
	{
		std::ostringstream str;
		str << "System not schedulable -";
		if (tmp_list.size() == schedule_vector.size())
		{
			str << " no element schedulable" << std::endl;
		}
		else
		{
			vslit--;
			str << " last schedulable element : " << std::endl;
...
void sca_synchronization_alg::reschedule_cluster(unsigned long cluster_id)
{
	...
	//-> may be the number of calls per period has been changed
...
void sca_synchronization_alg::sca_sync_objT::reinit()
{
	for (unsigned long ipnr = 0; ipnr < nin; ipnr++)
	{
		...
		//add inport delay (may be outport delay added before)
